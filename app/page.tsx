...
function ContactoInline() {
  ...
  if (enviado) return (
    <div className="flex-1 text-center py-4">
      <p className="text-white font-medium">✅ ¡Mensaje enviado! Te responderemos pronto.</p>
    </div>
  )
    ...
    <form onSubmit={handleSubmit} className="flex-1 space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <input
          ...
          required
          className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-sm text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-white/30"
        />
        <input
          type="email"
          placeholder="Email"
      ...
      <textarea
        ...
        required
        rows={3}
        className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-sm text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-white/30 resize-none"
      />
      <button
        type="submit"
        disabled={loading}
        className="w-full bg-white py-2 rounded-lg text-sm font-medium transition disabled:opacity-50 hover:bg-gray-100"
        style={{ color: PRIMARY }}
      >
...
export default function HomePage() {
  const cards = [
    ...
    },
    {
      ...
      desc: 'Envíanos tus restos de impresión y conviértelos en descuento para tus próximas compras. 2€ por kg reciclado.',
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-16">
        ...
        >
          <img
            ...
            className="mb-"
          />
          <p className="text-gray-400 text-sm tracking-widest uppercase">Filamentos · Economía circular · Sostenibilidad</p>
        </motion.div>
        ...
        <div className="max-w-5xl w-full mt-8">
          <motion.div
            ...
            className="rounded-2xl p-8 flex flex-col md:flex-row gap-8 items-start"
            style={{ backgroundColor: PRIMARY }}
          >
            <div className="flex-1">
              <h3 className="text-xl font-bold mb-2 text-white">¿Tienes alguna duda?</h3>
